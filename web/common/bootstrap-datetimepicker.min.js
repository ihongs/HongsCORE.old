 ...
 */
(function(a){a.fn.datetimepicker.dates.en={months:hsGetLang("date.LM"),monthsShort:hsGetLang("date.SM"),days:hsGetLang("date.LE"),daysShort:hsGetLang("date.SE"),daysMin:hsGetLang("date.SE"),meridiem:hsGetLang("time.Sa"),weekStart:hsGetLang("week.start"),today:hsGetLang("date.today"),format:_hs2bsDF(hsGetLang("datetime.format")),suffix:[]};a(document).on("click","[data-toggle=datetimepicker]",function(){if(a(this).data("datetimepicker")){return}var d=a(this);var b;var c={autoclose:true,todayBtn:true,todayHighlight:false};b=d.attr("data-position");if(b){c.pickerPosition=b}b=d.attr("data-format");if(b){c.format=_hs2bsDF(b)}else{if(d.is(".input-date")){c.format=_hs2bsDF(hsGetLang("date.format"))}else{if(d.is(".input-time")){c.format=_hs2bsDF(hsGetLang("time.format"))}else{c.format=_hs2bsDF(hsGetLang("datetime.format"))}}}if(/p/i.test(c.format)){c.showMeridian=true}if(/i/.test(c.format)){c.startView=0;c.minView=0;if(!/[md]/.test(c.format)){if(!/[Hh]/.test(c.format)){c.maxView=0}else{c.maxView=1}}}else{if(/h/i.test(c.format)){c.startView=1;c.minView=1;if(!/[md]/.test(c.format)){c.maxView=1}}else{if(/d/.test(c.format)){c.startView=2;c.minView=2}else{if(/m/.test(c.format)){c.startView=3;c.minView=3}else{if(/y/.test(c.format)){c.startView=4;c.minView=4}}}}}d.datetimepicker(c);d.datetimepicker("show")})}(jQuery));
