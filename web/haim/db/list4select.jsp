...
<%
    ...
    String select = lang.translate("fore.select.ensure", title);
%>
<h2><%=list%></h2>
<div id="hcim-list">
    ...
    <div>
        <div class="toolbox col-md-9 btn-group">
            <button type="button" class="create btn btn-primary"><%=select%></button>
        ...
        <table class="table table-hover table-striped">
            <thead>
                <tr>
                    ...
                    <th data-fn="<%=key%>" class="sortable text-right"><%=map.get("text")%></th>
                    <%} else if ("datetime".equals(map.get("type"))) {%>
                    <th data-fn="<%=key%>" data-ft="_datetime" class="sortable datetime"><%=map.get("text")%></th>
                    <%} else if ("date".equals(map.get("type"))) {%>
                    <th data-fn="<%=key%>" data-ft="_date" class="sortable date"><%=map.get("text")%></th>
                    <%} else if ("time".equals(map.get("type"))) {%>
                    <th data-fn="<%=key%>" data-ft="_time" class="sortable time"><%=map.get("text")%></th>
