...
考虑到构建一个框架推广实在太困难(我也不打算推广), 我正在此框架基础上搭建一个通用的信息管理系统, 类似 Django-Admin 那样的一个玩意, 但不需要编写代码, 仅在系统中通过设置便可搭建起属于自己的信息管理系统, 并自动产生应用接口供移动或 PC 客户端调用. 同时支持编写代码处理和扩展特殊的模块逻辑和界面. 另一个数据分析系统也在构思中(控制和报告部分在此基础上开发).
...
HongsCORE
即: Hong's Common Object Requesting Engine, 通用对象请求引擎, 拼凑的有些生硬. 在设计第一个原型框架时(PHP 版), 我买了一台 Intel Core CPU 的笔记本电脑, 当时随意的给她取了个名字叫 Core, 后来觉得名字应该更有意义才扩展成了以上缩写.
...
感谢 林叶,杨林,袁杰,赵征 等朋友过去对我的支持和宽容, 谢谢你们!
